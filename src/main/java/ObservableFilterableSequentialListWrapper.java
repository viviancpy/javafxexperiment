...
public final class ObservableFilterableSequentialListWrapper<E> extends ModifiableObservableListBase<E> implements ObservableList<E>, SortableList<E> {
    private final List<E> backingList;
    private final ElementObserver elementObserver;
    private SortHelper helper;

    public ObservableFilterableSequentialListWrapper(List<E> list) {
        backingList = list;
        elementObserver = null;
    }

    public ObservableFilterableSequentialListWrapper(List<E> list, Callback<E, Observable[]> extractor) {
        backingList = list;
    ...
    protected E doSet(int index, E element) {
        try {
            ...
            e.set(element);
            if (elementObserver != null){
                elementObserver.detachListener(oldVal);
                elementObserver.attachListener(element);
            }
            return oldVal;
        } catch (NoSuchElementException exc) {
    ...
    protected E doRemove(int index) {
        try {
            ...
            e.remove();
            if (elementObserver != null)
                elementObserver.detachListener(outCast);
            return outCast;
        } catch (NoSuchElementException exc) {
            throw new IndexOutOfBoundsException("Index: "+index);
        }
